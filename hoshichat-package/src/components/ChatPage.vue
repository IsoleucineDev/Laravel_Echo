      ...
      <div class="chat-area">
        <div v-if="selectedConversation" class="chat-content">
          <div class="chat-header">
            ...
            <div class="chat-status" :class="{ connected: wsConnected }">
          ...
          <div class="messages-container">
            ...
            >
              ...
              <div v-if="message.user_id === userId" class="message-actions">
                <button @click="startEditMessage(message)" class="btn-action">Editar</button>
                <button @click="handleDeleteMessage(message.id)" class="btn-action btn-delete">Eliminar</button>
              </div>
            </div>
    ...
    <div v-if="showNewConversationModal" class="modal-overlay" @click="showNewConversationModal = false">
      <div class="modal" @click.stop>
        ...
        <div class="modal-actions">
          <button @click="handleCreateConversation" class="btn-primary">Crear</button>
...
export default {
  ...
  props: {
    /** Displayed app title in the sidebar header */
    appName: {
      type: String,
      default: 'HoshiChat',
    },
    ...
    /**
     * Configured Laravel Echo instance (created with createEcho() from this package).
     * Pass the instance so that WebSocket auth uses the current token.
     */
    echoInstance: {
      type: Object,
      required: true,
  ...
  async mounted() {
    this.wsConnected = useWebSocket(this.echoInstance).isConnected();
  ...
  methods: {
    async loadConversations() {
      try {
        const response = await useConversations({ baseURL: this.apiBaseUrl, token: this.token }).fetchConversations();
        this.conversations = response.data;
      } catch (error) {
        console.error('[hoshichat] loadConversations:', error);
      }
    },
    ...
    async selectConversation(conversationId) {
      ...
      try {
        const response = await useConversations({ baseURL: this.apiBaseUrl, token: this.token }).getConversation(conversationId);
        this.selectedConversation = response.data;
        await this.loadMessages();
        this.setupWebSocketListeners();
      } catch (error) {
        console.error('[hoshichat] selectConversation:', error);
      }
    },

    async loadMessages() {
      try {
        const response = await useMessages({ baseURL: this.apiBaseUrl, token: this.token }).fetchMessages(this.selectedConversationId);
        this.messages = response.data;
      } catch (error) {
        console.error('[hoshichat] loadMessages:', error);
      }
    },

    setupWebSocketListeners() {
      useWebSocket(this.echoInstance).listenToConversation(this.selectedConversationId, {
        onMessageSent: (event) => { this.messages.push(event); },
    ...
    async sendMessage() {
      ...
      this.newMessage = '';
      try {
        await useMessages({ baseURL: this.apiBaseUrl, token: this.token }).createMessage(this.selectedConversationId, content);
      } catch (error) {
        this.newMessage = content;
        console.error('[hoshichat] sendMessage:', error);
      }
    },
    ...
    async handleDeleteMessage(messageId) {
      ...
      try {
        await useMessages({ baseURL: this.apiBaseUrl, token: this.token }).deleteMessage(messageId);
      } catch (error) {
        console.error('[hoshichat] deleteMessage:', error);
      }
    },
    ...
    startEditMessage(message) {
      ...
      if (newContent && newContent !== message.content) {
        useMessages({ baseURL: this.apiBaseUrl, token: this.token })
          .updateMessage(message.id, newContent)
          .catch((err) => console.error('[hoshichat] updateMessage:', err));
      }
    },
    ...
    async handleCreateConversation() {
      ...
      try {
        await useConversations({ baseURL: this.apiBaseUrl, token: this.token }).createConversation(this.newConversationData);
        this.newConversationData = { name: '', description: '' };
        this.showNewConversationModal = false;
        await this.loadConversations();
      } catch (error) {
        console.error('[hoshichat] createConversation:', error);
      }
    },

    logout() {
      useWebSocket(this.echoInstance).stopListeningToAll();
      this.$emit('logout');
    },
